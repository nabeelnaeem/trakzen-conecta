...
// iframe. The only script allowed in there is this one, which turns link
// clicks into a postMessage the parent forwards to the system browser.
//
...
export const FRAME_SCRIPT =
  "document.addEventListener('click',function(e){var t=e.target;var a=t&&t.closest?t.closest('a[href]'):null;if(!a)return;e.preventDefault();parent.postMessage({type:'tc-open',href:a.href},'*');});";
